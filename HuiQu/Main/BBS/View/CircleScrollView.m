...
@property (assign, nonatomic) NSTimeInterval textCount;     //文本数量
...
@property (strong, nonatomic) UIControl *grayControl;
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.grayControl = [[UIControl alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-30, self.bounds.size.width, 30)];
        self.grayControl.backgroundColor = [UIColor blackColor];
        self.grayControl.alpha = 0.6;
        [self addSubview:self.grayControl];
        CGSize size =  [TextTool getTextSizeWithFont:@"跟世界一同奔跑，你我同在。" withFont:[UIFont systemFontOfSize:12]];
        self.controlLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.bounds.size.width/2-size.width/2, 5, size.width, 20)];
        [self.controlLabel setText:@"跟世界一同奔跑，你我同在。"];
        [self.controlLabel setTextColor:[UIColor whiteColor]];
        [self.controlLabel setFont:[UIFont systemFontOfSize:12]];
        [self.grayControl addSubview:self.controlLabel];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.index = index;
    [_controlLabel setText:_titles[index]];
    return cell;
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    [_controlLabel setText:_titles[inc]];
    // 设置 PageControl
