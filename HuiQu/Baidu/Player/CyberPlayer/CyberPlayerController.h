...
/**
 百度媒体云播放器iOS SDK（以下简称“播放器iOS SDK”）是百度云推出的iOS平台视频播放器软件开发工具包（SDK），为iOS开发者提供简单、便捷的开发接口，帮助开发者在iPad/iPhone/iPod和Apple TV设备上实现媒体播放功能。播放器SDK内嵌百度自主研发的T5播放内核，提供简单、便捷的媒体应用开发能力，具有以下的特色功能：
...
+ (void) setBAEAPIKey:(NSString*)ak SecretKey:(NSString*)sk NS_DEPRECATED_IOS(5_0, 6_0);

/**
 @abstract 设置百度云开发者的Access key。
 @param ak 开发者的Access Key。
 @discussion CyberPlayerController与百度云的账户系统绑定。使用CyberPlayerController App需要传入开发者的Access Key参数。如果未传入Access Key，或者Access Key错误，播放器将在视频画面右上方打印告警信息，并且播放行为统计功能将失效。
 开发者需要在[百度云](http://bce.baidu.com)注册账户并且通过实名认证，方可获取对应的Access Key。具体信息，请参考[操作指南 管理ACCESSKEY](http://bce.baidu.com/doc/BOS/GettingStarted.html#.E7.AE.A1.E7.90.86ACCESSKEY)”的相关介绍。
...
/**
 @abstract 截图.
 @discussion 当播放器正在播放视频时，调用此方法。
 @since Available in CyberPlayerController 1.6.2 and later.
 @warning 在播放器的状态不是CBPMoviePlaybackStatePlaying,CBPMoviePlaybackStatePaused时，将返回nil。
 @see
 */
- (UIImage*)snapshot;
